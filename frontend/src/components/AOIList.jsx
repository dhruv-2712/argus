...
import { useScan, useDeleteAOI } from "../hooks/useArgusData"

export default function AOIList({ aois, onCreateClick, onAOISelect }) {
  const qc = useQueryClient()
  const scan = useScan()
  const del = useDeleteAOI()
  ...
  const toggle = async (aoi) => {
    try {
      ...
      // System AOIs are read-only server-side; surface nothing noisy.
      console.warn("Toggle failed:", e.message)
    }
  }

  const remove = async (aoi) => {
    if (!window.confirm(`Delete "${aoi.name}" and all its contacts? This cannot be undone.`)) return
    try {
      await del.mutateAsync(aoi.id)
    } catch (e) {
        ...
        {aois.map(a => (
          ...
          >
            ...
            <div className="flex items-center" style={{ gap: 7, marginBottom: 9 }}>
              ...
              {!a.owned && (
                <span className="mono" style={{ fontSize: 8, fontWeight: 700, letterSpacing: "0.1em", color: "var(--dim)", border: "1px solid var(--line)", padding: "1px 5px" }}>
                  SYSTEM
                </span>
              )}
            </div>
            <div className="flex" style={{ gap: 6 }}>
              ...
              {a.owned && (
                <button
                  ...
                  className="mono"
                  title="Delete area"
                  style={{ background: "transparent", border: "1px solid var(--line)", borderRadius: 0, padding: "4px 9px", fontSize: 9.5, color: "var(--red)", cursor: "pointer", display: "flex", alignItems: "center", gap: 5, fontWeight: 700, letterSpacing: "0.06em", marginLeft: "auto" }}
                >
                  <Trash2 size={11} />
                </button>
              )}
